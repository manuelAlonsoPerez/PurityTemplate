...
jaToolsInit = function ($) {
    ...
    $('div#toolbar-apply > button.button-apply').on('click', function () {
        var errorText = $('#system-message-container').find('h4.alert-heading');
        if (errorText.text().length){
            errorText.text('The form cannot be submitted as it\'s missing required data.\nPlease correct the marked fields and try again.')
        }
    })
};
